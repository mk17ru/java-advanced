package info.kgeorgiy.ja.kozlov.text;

import java.util.*;

public final class UsageResourceBundle_en
        extends ListResourceBundle {
    private final Object[][] CONTENTS = {
            {"analyzeFile", "Analyzed file"},
            {"words", "words"},
            {"numbers", "numbers"},
            {"summ", "sum"},
            {"sentenceStat", "Sentence statistics"},
            {"sentenceNumber", "Number of sentences"},
            {"sentenceAverage", "Average sentence length"},
            {"sentenceMin", "Minimum sentence"},
            {"sentenceMax", "Maximum sentence"},
            {"sentenceMinLength", "Minimum sentence length"},
            {"sentenceMaxLength", "Maximum sentence length"},
            {"fullStat", "Summary statistics"},
            {"wordsNumber", "Number of words"},
            {"numberNumber", "Number of numbers"},
            {"sumNumber", "Number of sums"},
            {"dateNumber", "Number of dates"},
            {"wordStat", "Word statistics"},
            {"wordNumber", "Word number"},
            {"diff", "different"},
            {"wordAverage", "Average word length"},
            {"wordMin", "Minimum word"},
            {"wordMax", "Maximum word"},
            {"wordMinLength", "Minimum word length"},
            {"wordMaxLength", "Maximum word length"},
            {"numberStat", "Statistics by numbers"},
            {"numberAverage", "Average"},
            {"numberMin", "Minimum number"},
            {"numberMax", "Maximum number"},

            {"sumStat", "Amount of money statistics"},
            {"sumAverage", "Average amount"},
            {"sumMin", "Minimum amount"},
            {"sumMax", "Maximum amount"},

            {"dateStat", "Date statistics"},
            {"dateAverage", "Average date"},
            {"dateMin", "Minimum amount"},
            {"dateMax", "Maximum date"}
    };

    protected Object[][] getContents() {
        return CONTENTS;
    }
}
